...
- 菜品封面上传后会在浏览器本地智能寻找主体，并支持拖动、焦点滑杆和 60%–220% 缩放微调；缩小时使用原图柔化背景自然补边，保存时会按预览画面真实生成统一比例封面并存入 NAS。粘贴网络图片地址后会实时通过同源安全代理预览，确认取景后转存 NAS，不依赖外站长期可用。
...
- 主厨端可切换“厨房今日营业 / 厨房今天休息”；状态保存在 NAS SQLite，朋友端同步显示绿灯或红灯。休息时仍可浏览完整菜单，但普通菜单和专属邀请都不能加菜、结算或提交订单。
- 餐桌日记拥有独立主厨入口：普通点菜和专属邀请只要进入开饭状态都可以选填一篇，支持照片、反复编辑和删除；删除日记不会删除原饭局，之后仍可重新补写。
- 微信链接卡片使用独立的 800×800 正方形轻量分享图，并为 Open Graph、Twitter 和通用抓取器输出 HTTPS 绝对地址，避免 NAS 反向代理的内部 HTTP 协议导致缩略图被丢弃。
