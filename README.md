...
登录 `/chef` 后，在“数据迁移”工具中可以“导出全部数据”或“导入备份”。导出 ZIP 包含 `export.json`、SQLite 业务数据以及 `/data/uploads/` 下的全部照片和元数据；导入会先创建当前数据库备份，再在事务中整体替换业务表并替换图片。`app_settings` 采用按 key 合并，密码、会话密钥和 API Key 等运行时敏感配置不会被导入覆盖。导入压缩包仅接受 `export.json` 与 `uploads/` 路径，并会拒绝目录穿越。
