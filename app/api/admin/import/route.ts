...
function importUploadPath(uploadId: string) {
  ...
  return path.join(getDataDir(), "import-staging", uploadId);
}

async function readImportUploadManifest(uploadDir: string) {
  try {
    return JSON.parse(await readFile(path.join(uploadDir, "manifest.json"), "utf8")) as ImportUploadManifest;
  } catch {
    throw new ImportInputError("导入上传会话已失效，请重新选择 ZIP");
  }
}
...
async function receiveImportChunk(request: Request) {
  ...
  try {
    manifest = await readImportUploadManifest(uploadDir);
    if (manifest.totalBytes !== totalBytes || manifest.totalChunks !== totalChunks) throw new ImportInputError("导入分块与当前文件不匹配");
  } catch (error) {
    if (!(error instanceof ImportInputError) || error.message.includes("会话已失效")) {
...
async function assembleImportChunks(uploadDir: string, manifest: ImportUploadManifest) {
  ...
  const assembledPath = path.join(uploadDir, "archive.zip");
  let total = 0;
  for (let index = 0; index < manifest.totalChunks; index += 1) {
    ...
    await pipeline(createReadStream(chunkPath), createWriteStream(assembledPath, { flags: index === 0 ? "w" : "a", mode: 0o600 }));
  }
  if (total !== manifest.totalBytes) throw new ImportInputError("导入文件大小校验失败");
  return readFile(assembledPath);
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (chunkUploadId && isFinalChunk) {
      ...
      const manifest = await readImportUploadManifest(chunkUploadDir);
